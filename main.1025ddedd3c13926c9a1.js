...
var code = "<!DOCTYPE html> <html lang=\"en\"> <head> <meta charset=\"UTF-8\"/> <meta http-equiv=\"X-UA-Compatible\" content=\"IE=edge\"/> <meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"/> <title>Document</title> </head> <body> <header class=\"header\"> <nav class=\"header__navbar\"> <div class=\"header__navbar__navbar__title-logo\"> <p class=\"logo\">Bookshop</p> </div> <div class=\"navbar__nav\"> <a class=\"nav__item-link nav__item-link_active\" href=\"#\">Books</a> <a class=\"nav__item-link\" href=\"#\">Audiobooks</a> <a class=\"nav__item-link\" href=\"#\">Stationery &amp gifts</a> <a class=\"nav__item-link\" href=\"#\">blog</a> </div> <div class=\"icon-navbar\"> <img class=\"icon-navbar__icon-img\" src=\"" + ___HTML_LOADER_REPLACEMENT_0___ + "\" alt=\"icon-user\"/> <img class=\"icon-navbar__icon-img\" src=\"" + ___HTML_LOADER_REPLACEMENT_1___ + "\" alt=\"icon-search\"/> <img class=\"icon-navbar__icon-img\" src=\"" + ___HTML_LOADER_REPLACEMENT_2___ + "\" alt=\"icon-shop\"/> </div> </nav> </header> <main> <div class=\"wrapper\"> <div class=\"content__miniCard_second\"> <p class=\"title__card\"> top<br/> 100<br/>Books<br/>2022 </p> <img class=\"arrow-right\" src=\"" + ___HTML_LOADER_REPLACEMENT_3___ + "\" alt=\"arrow-right\"/> </div> </div> <div class=\"content\"> <div class=\"content__miniCards\"> <div class=\"content__miniCard_first\"> <p class=\"title__card\"> Change<br/> old book<br/>on new </p> <img class=\"arrow-right\" src=\"" + ___HTML_LOADER_REPLACEMENT_3___ + "\" alt=\"arrow-right\"/> </div> </div> <div class=\"main-container-block-1\"> </div> <div class=\"content__slider-dots\"></div> <footer></footer> </div></main> </body> </html> ";
// Exports
...
function initSlider(options) {
  ...
  function initImages() {
    script_slider_images.forEach((image, index) => {
      let imageDiv = `<div class="content__main-content image n${index} ${index === 0 ? "active" : ""}" style="background-image:url(${script_slider_images[index].url})" data-index="${index}">
         <h1 class="content__main-content__slider-title">${script_slider_images[index].title}</h1>
  ...
  function changeInfo(num) {
    if (!script_slider_images[num].city) return;
    let infoBox = document.querySelector(".box-information");
    let nameCity = infoBox.querySelector("#nameCity");
    nameCity.innerHTML = "";
    nameCity.innerHTML = `
                <h3>CITY:</h3>
                <span id="name-city" >${script_slider_images[num].city}</span>
            `;
    if (!script_slider_images[num].area) return;
    let area = infoBox.querySelector("#area");
    area.innerHTML = " ";
    area.innerHTML = `
                <h3>apartment area:</h3>
                <span class="m2"> ${script_slider_images[num].area} m<sup>2</sup></span>
            `;
    if (!script_slider_images[num].time) return;
    let time = infoBox.querySelector("#time");
    time.innerHTML = " ";
    time.innerHTML = `
            <h3>Repair time:</h3>
            <span id="repair-time">${script_slider_images[num].time}</span>
            `;
  }
